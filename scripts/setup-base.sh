...
for SITE in `ls -d web/sites/*/`; do
  ...
  if [[ ! -d web/sites/$SITE/files ]]; then
    ...
    if [ -n $ENV_UNIX_GROUP_WEBSERVER ]; then
      chown :$ENV_UNIX_GROUP_WEBSERVER web/sites/$SITE/files -R
      chgrp :$ENV_UNIX_GROUP_WEBSERVER web/sites/$SITE/files -R
    fi
  fi
  ...
  if [[ ! -d web/sites/$SITE/files-private ]]; then
    ...
    if [ -n $ENV_UNIX_GROUP_WEBSERVER ]; then
      chown :$ENV_UNIX_GROUP_WEBSERVER web/sites/$SITE/files-private -R
      chgrp :$ENV_UNIX_GROUP_WEBSERVER web/sites/$SITE/files-private -R
    fi
  fi
